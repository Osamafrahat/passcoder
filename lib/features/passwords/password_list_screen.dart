...
class PasswordListScreenState extends State<PasswordListScreen> {
  ...
  void openSearch() => Navigator.push(context, MaterialPageRoute(builder: (_) => const GlobalSearchScreen()));

  Future<void> _toggleFavorite(PasswordModel p) async {
    try { await _supabase.from('passwords').update({'is_favorite': !p.isFavorite}).eq('id', p.id); loadPasswords(); } catch (_) {}
  }
  ...
  Future<void> _softDelete(PasswordModel p) async {
    try {
      ...
      try { await _supabase.from('passwords').delete().eq('id', p.id); loadPasswords(); } catch (_) {}
    }
  }
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return _PasswordCard(
                            ...
                            onQuickCopy: () async {
                              try { final decrypted = await _encryption.decryptData(p.passwordEncrypted); _quickCopy(decrypted, 'Password'); } catch (_) { _quickCopy('Decryption failed', 'Password'); }
                            },
...
class _PasswordCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Material(
        color: theme.colorScheme.surface, borderRadius: BorderRadius.circular(18), elevation: 0,
        child: InkWell(
